import { DEFAULT_MAX_ARGUMENT_ARRAY_SIZE } from '../provider';
...
import { TITLE_METADATA_KEY, DESCRIPTION_METADATA_KEY, KEYWORD_METADATA_KEY_PREFIX, LAST_UPDATE_KEY, } from './const';
 ...
 */
export function extractWebsiteMetadata(metadata) {
 ...
 */
export async function getMultipleSitesGlobalMetadata(addresses, provider, isFinal = false) {
    const publicAPI = await PublicAPI.fromProvider(provider);
    // get all global metadata keys for all addresses
    const addressesMetadataKeysList = await getMultipleAddressesDatastoreKeys(publicAPI, addresses.map((address) => ({
        address,
    ...
    const allMetadataEntries = await batchListAndCall(metadataKeysList, async (metadataKeysBatch) => {
    ...
    const metadata = allMetadataEntries
        ...
        const metadataKeyBytes = metadataKeysList[index].key.slice(GLOBAL_METADATA_TAG.length);

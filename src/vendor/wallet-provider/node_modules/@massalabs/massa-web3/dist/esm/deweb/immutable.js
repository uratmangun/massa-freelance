import { batchListAndCall } from '../operation';
 ...
 */
export async function areImmutables(provider, contractAddresses, isFinal = false) {
    if (contractAddresses.length === 0) {
        return [];
    }
    const publicAPI = await PublicAPI.fromProvider(provider);
    const bytecodes = await batchListAndCall(contractAddresses, async (contractAddressesBatch) => {
 ...
 */
export async function isImmutable(address, provider, isFinal = false) {

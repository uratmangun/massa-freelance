...
exports.getMultipleAddressesDatastoreKeys = void 0;
 ...
 */
async function getMultipleAddressesDatastoreKeys(client, params) {
    ...
    while (toContinue) {
        ...
        const batchedIndices = (0, batchOpArrayParam_1.toBatch)(activeIndices, constants_1.DEFAULT_MAX_ARGUMENT_ARRAY_SIZE);
        // Process each batch
        for (const batchIndices of batchedIndices) {
            const requests = batchIndices.map((index) => {
                ...
                return {
                    ...
                    maxCount: constants_1.DEFAULT_GET_DATASTORE_KEYS_PAGE_SIZE,
                };
            });
            let responses;
            try {
            ...
            for (let i = 0; i < batchIndices.length; i++) {
                ...
                if (!response || response.keys.length === 0) {
                    ...
                    if (response.keys.length < constants_1.DEFAULT_GET_DATASTORE_KEYS_PAGE_SIZE) {
                        addressStates[stateIndex].hasMoreKeys = false;
                    }
                    else {
...
exports.getMultipleAddressesDatastoreKeys = getMultipleAddressesDatastoreKeys;

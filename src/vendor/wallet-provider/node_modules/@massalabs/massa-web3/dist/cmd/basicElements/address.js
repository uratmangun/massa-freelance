...
(function (AddressType) {
    AddressType[AddressType["EOA"] = 0] = "EOA";
    AddressType[AddressType["Contract"] = 1] = "Contract";
...
function getPrefix(str) {
    ...
    throw new Error(`invalid address prefix: one of ${expected.join(' or ')} was expected.`);
}
/**
...
class Address {
    ...
    static initFromVersion(version = DEFAULT_VERSION) {
        switch (version) {
            case versioner_1.Version.V0:
                return new Address(new base58_1.default(), new varintVersioner_1.default(), version);
            default:
    ...
    static fromString(str) {
        try {
            ...
            const versionedBytes = address.serializer.deserialize(str.slice(prefix.length));
            const { version: extractedVersion } = address.versioner.extract(versionedBytes);
            // safety check
            if (extractedVersion !== version) {
                throw new Error(`invalid version: ${version}. ${address.version} was expected.`);
            }
            address.bytes = Uint8Array.from([
                ...varint_1.default.encode(address.isEOA ? AddressType.EOA : AddressType.Contract),
                ...versionedBytes,
            ]);
            return address;
        }
    ...
    getType() {
        ...
        return varint_1.default.decode(this.bytes);
    }
    /**
    ...
    static fromPublicKey(publicKey) {
        if (publicKey.version !== versioner_1.Version.V0) {
            throw new Error(`unsupported public key version: ${publicKey.version}`);
        }
        const address = Address.initFromVersion(versioner_1.Version.V0);
        const rawBytes = publicKey.hasher.hash(publicKey.toBytes());
        address.bytes = Uint8Array.from([
            AddressType.EOA,
            ...address.versioner.attach(versioner_1.Version.V0, rawBytes),
        ]);
        address.isEOA = true;
        return address;
    }
    ...
    static fromBytes(bytes) {
        ...
        const versionedBytes = bytes.slice(varint_1.default.encodingLength(addressType));
        const { version: extractedVersion } = address.versioner.extract(versionedBytes);
        if (extractedVersion !== version) {
            throw new Error(`invalid version: ${version}. ${address.version} was expected.`);
        }
    ...
    toString() {
        ...
        const versionedBytes = this.bytes.slice(varint_1.default.encodingLength(this.getType()));
        return `${this.isEOA ? ADDRESS_USER_PREFIX : ADDRESS_CONTRACT_PREFIX}${this.serializer.serialize(versionedBytes)}`;
    }
    /**
     ...
     */
    static extractFromBuffer(data, offset = 0) {
        // addr type
        varint_1.default.decode(data, offset);
        const typeLen = varint_1.default.decode.bytes;
        if (typeLen === undefined) {
            throw new Error('invalid address: type not found.');
        }
        // version
        varint_1.default.decode(data, offset + typeLen);
        const versionLen = varint_1.default.decode.bytes;

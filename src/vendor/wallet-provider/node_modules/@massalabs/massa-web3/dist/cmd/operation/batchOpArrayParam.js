...
exports.batchListAndCall = exports.toBatch = void 0;
/**
 ...
 */
function toBatch(array, batchSize) {
    if (batchSize < 0) {
 ...
 */
async function batchListAndCall(list, batchFunction, batchSize) {
    // Batch the input list
    const batchedList = toBatch(list, batchSize);
    // Process all batches in parallel
    const batchResults = await Promise.all(batchedList.map(async (batch) => {
        return batchFunction(batch);
    }));
    // Flatten all results into a single array
    return batchResults.flat();
}
exports.batchListAndCall = batchListAndCall;

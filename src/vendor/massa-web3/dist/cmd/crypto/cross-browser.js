...
exports.aesGCMDecrypt = exports.aesGCMEncrypt = exports.pbkdf2 = void 0;
...
async function pbkdf2Node(password, salt, opts) {
    const { iterations, keyLength, hash } = opts;
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const crypto = require('crypto');
    return new Promise((resolve, reject) => {
        crypto.pbkdf2(password, salt, iterations, keyLength, hash, (err, derivedKey) => {
            if (err)
                reject(err);
            else
                resolve(derivedKey);
...
async function pbkdf2Browser(password, salt, opts) {
    const { iterations, keyLength, hash } = opts;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const crypto = window.crypto || window.msCrypto;
    if (!crypto)
        throw new Error('Your browser does not expose window.crypto.');
    ...
    const derivedKey = await crypto.subtle.deriveKey({
        ...
        hash: { name: hash },
    }, keyMaterial, { name: 'AES-GCM', length: keyLength * U8_SIZE_BITS }, false, ['encrypt', 'decrypt']);
 ...
 */
async function pbkdf2(password, salt, opts) {
    if (isNode()) {
...
async function aesGCMEncrypt(data, key, iv) {
    ...
    const keyData = await window.crypto.subtle.importKey('raw', key, { name: 'AES-GCM' }, false, ['encrypt']);
    const encrypted = await window.crypto.subtle.encrypt({ name: 'AES-GCM', iv: iv }, keyData, data);
 ...
 */
async function aesGCMDecrypt(encryptedData, key, iv) {
    if (key.length !== KEY_SIZE_BYTES) {
        throw new Error(`key must be ${KEY_SIZE_BYTES} bytes`);
    }
    if (iv.length !== IV_SIZE_BYTES) {
    ...
    if (isNode()) {
        ...
        decipher.setAuthTag(encryptedData.slice(encryptedData.length - AUTH_TAG_SIZE_BYTES));
        const decrypted = Buffer.concat([
            decipher.update(encryptedData.slice(0, encryptedData.length - AUTH_TAG_SIZE_BYTES)),
            decipher.final(),
        ]);
        return decrypted;
    }
    const keyData = await window.crypto.subtle.importKey('raw', key, { name: 'AES-GCM' }, false, ['decrypt']);
    const decrypted = await window.crypto.subtle.decrypt({ name: 'AES-GCM', iv: iv }, keyData, encryptedData);

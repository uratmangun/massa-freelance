...
class PublicAPI {
    ...
    async executeReadOnlyBytecode(params) {
        return this.connector
            .execute_read_only_bytecode([(0, _1.formatReadOnlyExecuteSCParams)(params)])
            .then((r) => (0, _1.formatReadOnlyExecuteSCResponse)(r[0]));
    }
    async executeMultipleReadOnlyBytecode(executeReadonlyParams) {
        const params = executeReadonlyParams.map((param) => (0, _1.formatReadOnlyExecuteSCParams)(param));
        const res = await this.connector.execute_read_only_bytecode(params);
        return res.map((r) => (0, _1.formatReadOnlyExecuteSCResponse)(r));
    }

...
export function formatNodeStatusObject(status) {
    return {
        config: {
...
export function formatReadOnlyCallResponse(res) {
    return {
...
export function formatReadOnlyExecuteSCParams(params) {
    const res = {
...
export function formatReadOnlyExecuteSCResponse(res) {
    return {
        value: res.result.Ok ? new Uint8Array(res.result.Ok) : new Uint8Array(),
        gasCost: res.gas_cost,
        error: res.result.Error,
...
export function serializeDatastore(data) {

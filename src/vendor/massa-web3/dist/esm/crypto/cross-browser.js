 ...
 */
export async function pbkdf2(password, salt, opts) {
    if (isNode()) {
 ...
 */
export async function aesGCMEncrypt(data, key, iv) {
 ...
 */
export async function aesGCMDecrypt(encryptedData, key, iv) {
    if (key.length !== KEY_SIZE_BYTES) {
        throw new Error(`key must be ${KEY_SIZE_BYTES} bytes`);
    }
    if (iv.length !== IV_SIZE_BYTES) {

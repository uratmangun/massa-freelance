...
import { NetworkName, parseCallArgs } from '../../utils';
import { CHAIN_ID } from '../../utils';
import { Mas, strToBytes } from '../../basicElements';
import { OperationStatus } from '../../operation';
// Import from api
import { AddressBalanceCandidate, AddressBalanceFinal, AddressDatastoreKeysCandidate, AddressDatastoreKeysFinal, ExecuteReadOnlyCallRequest, ExecutionQueryExecutionStatus, ExecutionQueryRequestItem, GetBlocksRequest, GetDatastoreEntriesRequest, GetDatastoreEntryFilter, GetEndorsementsRequest, GetNextBlockBestParentsRequest, GetOperationABICallStacksRequest, GetOperationsRequest, GetScExecutionEventsRequest, GetSelectorDrawsRequest, GetSlotABICallStacksRequest, GetSlotTransfersRequest, GetStakersRequest, GetStatusRequest, GetTransactionsThroughputRequest, NewBlocksFilter, NewBlocksServerRequest, NewEndorsementsFilter, NewEndorsementsServerRequest, NewFilledBlocksFilter, NewFilledBlocksServerRequest, NewOperationsFilter, NewOperationsServerRequest, NewSlotExecutionOutputsServerRequest, OpExecutionStatusCandidate, QueryStateRequest, ScExecutionEventsFilter, SearchBlocksFilter, SearchBlocksRequest, SearchEndorsementsFilter, SearchEndorsementsRequest, SearchOperationsFilter, SearchOperationsRequest, SelectorDrawsFilter, StakersFilter, } from '../../generated/grpc/public_pb';
import { OperationIds, OpTypes, } from '../../generated/grpc/massa/model/v1/operation_pb';
import { Slot, SlotRange } from '../../generated/grpc/massa/model/v1/slot_pb';
import { EndorsementIds, } from '../../generated/grpc/massa/model/v1/endorsement_pb';
import { Addresses } from '../../generated/grpc/massa/model/v1/address_pb';
import { BlockIds, } from '../../generated/grpc/massa/model/v1/block_pb';
...
export class GrpcPublicProvider {
    client;
    url;
    ...
    newEndorsementsStream(addresses, endorsementIds, blockIds) {
        ...
        if (endorsementIds) {
            ...
            filter.setEndorsementIds(new EndorsementIds().setEndorsementIdsList(endorsementIds));
            request.addFilters(filter);
        }
        if (addresses) {
            const filter = new NewEndorsementsFilter();
    ...
    newOperationsStream(addresses, operationIds, types) {
        ...
        if (operationIds) {
            ...
            filter.setOperationIds(new OperationIds().setOperationIdsList(operationIds));
            request.addFilters(filter);
        }
        if (addresses) {
            const filter = new NewOperationsFilter();
     ...
     */
    queryState(queries) {
        return this.client.queryState(new QueryStateRequest().setQueriesList(queries));
    }
    ...
    async getSlotABICallStacks(slots) {
        const grpcSlots = slots.map((slot) => new Slot().setPeriod(slot.period).setThread(slot.thread));
    ...
    async getStorageKeys(address, filter, final) {
        ...
        try {
            ...
            const ret = new ExecutionQueryRequestItem();
            if (final) {
                ret.setAddressDatastoreKeysFinal(new AddressDatastoreKeysFinal().setAddress(address).setPrefix(prefix));
            }
            else {
                ret.setAddressDatastoreKeysCandidate(new AddressDatastoreKeysCandidate()
                    .setAddress(address)
                    .setPrefix(prefix));
            }
            queries.push(ret);
            ...
            if (addressInfo.hasResult() && addressInfo.getResult()?.hasVecBytes()) {
                return (addressInfo
                    ...
                    ?.getVecBytes()
                    ?.getItemsList()
                    .map((item) => typeof item === 'string' ? strToBytes(item) : item) ?? []);
            }
    ...
    async readStorage(address, keys, final = true) {
        const filters = keys.map((item) => {
            const key = typeof item === 'string' ? strToBytes(item) : item;
    ...
    async getEvents(filter) {
        ...
        if (filter.start !== undefined && filter.end !== undefined) {
            ...
            ret.setSlotRange(new SlotRange()
                .setStartSlot(new Slot()
    ...
    async getNodeStatus() {
        ...
        const nodeStatusInfo = {
            config: {
                blockReward: Mas.toString(BigInt(status.getConfig()?.getBlockReward()?.getMantissa() ?? 0)),
                ...
                rollPrice: Mas.toString(BigInt(status.getConfig()?.getRollPrice()?.getMantissa() ?? 0)),
            ...
            minimalFees: Mas.toString(BigInt(status.getMinimalFees()?.getMantissa() ?? 0)),

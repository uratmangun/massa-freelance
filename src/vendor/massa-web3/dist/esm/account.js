...
import { PrivateKey, PublicKey } from './basicElements';
/**
...
export class Account {
     ...
     */
    static async fromPrivateKey(key, version = Version.V1) {
    ...
    static async fromKeyStore(keystore, password) {
        switch (keystore.Version) {
            case Version.V0: {
                ...
                const passwordSeal = new PasswordSeal(password, keystore.Salt, keystore.Nonce);
                ...
                const publicKey = PublicKey.fromBytes(Uint8Array.from(keystore.PublicKey));
                const address = publicKey.getAddress();
